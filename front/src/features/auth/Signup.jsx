...
export const signupAPI = createAsyncThunk(
  'auth/signup',
  async ({ email, first_name, last_name, password1, password2 }, { rejectWithValue }) => {
    try {
      const config = { headers: { "Content-Type": "application/json" } };
      const body = JSON.stringify({ email, first_name, last_name, password1, password2 });

      const res = await axios.post(`${API_URL}/registration/`, body, config)
...
const Signup = () => {
  ...
  useEffect(() => {
    setFormData({ email: "", first_name: "", last_name: "", password1: "", password2: "" })
  }, [success])
  ...
  const onSubmit = e => {
    e.preventDefault()
    dispatch(signupAPI({ email: email.toLowerCase(), first_name: first_name, last_name: last_name, password1: password1, password2: password2 }))
    window.scrollTo(0, 0);
  }
    ...
    <Layout>
      <div className="min-h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Crete your account</h2>
        </div>
          ...
          <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            ...
            <form onSubmit={e => onSubmit(e)} className="space-y-6">
              ...
              <div>
                ...
                >
                  {loading ? <ThreeDots color="#FFF" width={25} height={25} /> : 'Sign up'}
                </button>
              </div>
            </form>
